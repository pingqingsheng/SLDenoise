...
def main(args):
    ...
    ood_loader = ImageNet32(dataset=args.dataset, train=True)
    ...
    _count = 0
    _best_raw_l1 = torch.tensor(np.inf)
    _best_raw_ece = torch.tensor(np.inf)
    _best_ts_l1 = torch.tensor(np.inf)
    _best_ts_ece = torch.tensor(np.inf)
    _best_cali_acc = 0
    ...
    for epoch in range(N_EPOCH_INNER_CLS):
        ...
        if not (epoch % MONITOR_WINDOW):
            ...
            valid_total = 0
            valid_total = 0
            model_cls_cali = lula_calibrate_model(model_cls, 
            ...
            model_cls.eval()
            model_cls_cali.base_model.module.eval()
    ...
    if args.figure:
        ...
        plt.plot(x_axis[:-1], _valid_acc_cali_record[:-1], linewidth=2, label="Lula")
        plt.title('Acc Curve')
        plt.subplot(2, 2, 3)
        plt.plot(x_axis[:-1], _ece_raw_record[:-1], linewidth=2, label="Raw")
        plt.plot(x_axis[:-1], _ece_cali_record[:-1], linewidth=2, label="Lula")
        plt.legend()
        plt.title('ECE Curve')
        plt.subplot(2, 2, 4)
        plt.plot(x_axis[:-1], _l1_raw_record[:-1], linewidth=2, label="Raw")
        plt.plot(x_axis[:-1], _l1_cali_record[:-1], linewidth=2, label="Lula")
        plt.legend()
        plt.title('L1 Curve')

        time_stamp = datetime.datetime.strftime(datetime.datetime.today(), "%Y-%m-%d")
        fig.savefig(os.path.join("./figures", f"exp_log_{args.dataset}_{args.noise_type}_{args.noise_strength}_Lula_plot_{time_stamp}.png"))

    # >>> Debugging
    import pickle as pkl
    
    debug_folder = "../debug/lula"
    os.makedirs(debug_folder, exist_ok=True)
    time_stamp = datetime.datetime.strftime(datetime.datetime.today(), "%Y%m%d%H%M%S")
    debuglogger = os.path.join(debug_folder, f"lula_{args.dataset}_{args.noise_type}_{args.noise_strength}_{time_stamp}.pkl")
    print(debuglogger)
...
def lula_calibrate_model(model: torch.nn.modules, 
                         train_loader: DataLoader, 
                         valid_loader: DataLoader, 
                         ood_loader: DataLoader, 
                         device: torch.cuda.device)-> lula_utils.LULAModel:
    
    model_cls = copy.deepcopy(model)
    
    var0 = torch.tensor(1/(5e-4*len(train_loader.dataset))).float().cuda()
    nll  = torch.nn.CrossEntropyLoss(reduction='mean')
    n_units_grid = [[4], [8], [16], [32]]
    best_loss_noise = float('inf')
    
    best_model_statedict = model_cls.state_dict()
    
    model_cls.eval()
    for n_unit in n_units_grid:
        
        n_lula_units = n_unit
        model_cali = lula_utils.LULAModel(model_cls, n_lula_units).to(device)
        model_cali.enable_grad_mask()
        
        ood_loader.dataset.offset = np.random.randint(len(ood_loader.dataset))
        model_cali = lula_utils.train_lula_layer(
            model_cali, nll, valid_loader, ood_loader, 1/var0, 
            lr=LR, n_iter=1, progressbar=False, mc_samples=10 
        )
        
        model_cali.disable_grad_mask()
        model_cali.eval()
        model_cali.unmask()
        model_cali.base_model.module.fc.weight.requires_grad = False
        
        curr_model_statedict = model_cali.state_dict()
        
        # Do a LA over the trained LULA-augmented network
        model_kfla = lula_utils.KFLA(model_cali)
        model_kfla.get_hessian(train_loader)
        model_kfla.estimate_variance(var0)
        out_in  = lula_utils.kfla_predict(valid_loader, model_kfla, n_samples=10)
        out_out = lula_utils.kfla_predict(ood_loader,   model_kfla, n_samples=10, n_data=2000)
        
        h_in  = dist.Categorical(out_in).entropy().mean().cpu().numpy()
        h_out = dist.Categorical(out_out).entropy().mean().cpu().numpy()
        loss = h_in - h_out
        
        if loss < best_loss_noise:
            best_model_statedict = curr_model_statedict
    
    model_cali.load_state_dict(best_model_statedict)
...
if __name__ == "__main__":
    ...
    save_folder = os.path.join("../exp_logs/lula_" + dir_date)
    os.makedirs(save_folder, exist_ok=True)
    save_file_name = 'lula_' + datetime.date.today().strftime("%d_%m_%Y") + f"{args.dataset}_{args.noise_type}_{args.noise_strength}.json"
