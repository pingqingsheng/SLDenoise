...
def main(args):
    ...
    y_test  = copy.deepcopy(testset.targets)
    ...
    debug_folder = "../debug/ours"
    os.makedirs(debug_folder, exist_ok=True)
    time_stamp = datetime.datetime.strftime(datetime.datetime.today(), "%Y%m%d%H%M%S")
    debuglogger = os.path.join(debug_folder, f"ours_{args.dataset}_{args.noise_type}_{args.noise_strength}_{time_stamp}.pkl")
    print(debuglogger)
    ...
    return l1_loss_raw.item(), ece_loss_raw.item(), l1_loss_cali.item(), ece_loss_cali.item(), _best_cali_l1, _best_cali_ece, test_acc_cali, _best_cali_acc
...
if __name__ == "__main__":
    ...
    save_file_name = 'oursweightedv2_' + datetime.date.today().strftime("%d_%m_%Y") + f"{args.dataset}_{args.noise_type}_{args.noise_strength}.json"
    save_file_name = os.path.join(save_folder, save_file_name)
    print(save_file_name)
    
    for seed in [77, 78, 79]:
        args.seed = seed
        _, _, ours_l1,  ours_ece, best_l1, best_ece, ours_acc, best_acc = main(args)
        exp_config['l1'].append(ours_l1)
        exp_config['ece'].append(ours_ece)
        exp_config['acc'].append(ours_acc)
