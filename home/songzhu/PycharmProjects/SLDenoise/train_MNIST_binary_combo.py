...
from utils.noise import perturb_eta, noisify_with_P, noisify_mnist_asymmetric
from utils.utils import lrt_correction


# Experiment Setting Control Panel
N_EPOCH_OUTER: int = 1
N_EPOCH_INNER_CLS: int = 3
N_EPOCH_INNER_CONF: int = 20
CONF_RECORD_EPOCH: int = 1
LR: float = 1e-3
...
def main(args):
    ...
    if args.noise_type=='linear':
        ...
        y_tilde_train, P, _ = noisify_with_P(np.array(copy.deepcopy(y_train)), nb_classes=10, noise=args.noise_strength)
        eta_tilde_train = np.matmul(eta_train, P)
        eta_tilde_valid = np.matmul(eta_valid, P)
        eta_tilde_test  = np.matmul(eta_test, P)
        trainset.update_labels(y_tilde_train)
    elif args.noise_type=='asymmetric':
        y_tilde_train, P, _ = noisify_mnist_asymmetric(np.array(copy.deepcopy(y_train)), nb_classes=10, noise=args.noise_strength)
        eta_tilde_train = np.matmul(eta_train, P)
        eta_tilde_valid = np.matmul(eta_valid, P)
        eta_tilde_test = np.matmul(eta_test, P)
        trainset.update_labels(y_tilde_train)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--noise_type", type=str, help="Noise type", default='linear', choices={"linear", "uniform", ""})
