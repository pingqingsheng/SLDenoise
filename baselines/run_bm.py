...
def main(args):
    ...
    criterion_cls = BeliefMatchingLoss()
    ...
    _best_cali_acc = torch.tensor(0)
    _best_ts_l1_epoch = 0
    _best_ts_ece_epoch = 0
    ...
    if args.figure:
        ...
        plt.subplot(2, 2, 2)
        plt.plot(x_axis[:-1], _valid_acc_cali_record[:-1], linewidth=2, label="BM")
        plt.title('Acc Curve')
        plt.subplot(2, 2, 3)
        plt.plot(x_axis[:-1], _ece_cali_record[:-1], linewidth=2, label="BM")
        plt.legend()
        plt.title('ECE Curve')
        plt.subplot(2, 2, 4)
        plt.plot(x_axis[:-1], _l1_cali_record[:-1], linewidth=2, label="BM")
        plt.legend()
        plt.title('L1 Curve')

        time_stamp = datetime.datetime.strftime(datetime.datetime.today(), "%Y-%m-%d")
        fig.savefig(os.path.join("./figures", f"exp_log_{args.dataset}_{args.noise_type}_{args.noise_strength}_bm_plot_{time_stamp}.png"))

    # >>> Debugging
    import pickle as pkl
    debug_folder = "../debug/bm"
    os.makedirs(debug_folder, exist_ok=True)
    time_stamp = datetime.datetime.strftime(datetime.datetime.today(), "%Y%m%d%H%M%S")
    debuglogger = os.path.join(debug_folder, f"bm_{args.dataset}_{args.noise_type}_{args.noise_strength}_{time_stamp}.pkl")
    print(debuglogger)
...
class BeliefMatchingLoss(torch.nn.Module):
    def __init__(self, coeff=1e-2, prior=1.):
...
if __name__ == "__main__":
    ...
    save_folder = os.path.join("../exp_logs/bm_" + dir_date)
    os.makedirs(save_folder, exist_ok=True)
    save_file_name = 'bm_' + datetime.date.today().strftime("%d_%m_%Y") + f"{args.dataset}_{args.noise_type}_{args.noise_strength}.json"
