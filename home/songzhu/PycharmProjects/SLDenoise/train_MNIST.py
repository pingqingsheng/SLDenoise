...
def main(args):
    ...
    for outer_epoch in range(N_EPOCH_OUTER):
        ...
        for inner_epoch in range(N_EPOCH_INNER):
            ...
            if (not inner_epoch%MONITOR_WINDOW) and (inner_epoch>0):
                valid_correct = 0
                valid_total = 0
                model_cls.eval()
        ...
        for inner_epoch in range(N_EPOCH_INNER):
            ...
            if (not inner_epoch%MONITOR_WINDOW) and (inner_epoch>0):
                ...
                for _, (indices, images, _, delta_eta) in enumerate(tqdm(valid_loader, ascii=True, ncols=100)):
                    ...
                    valid_loss_delta_eta+=criterion_conf(outs, delta_eta_valid)

                valid_mse = valid_loss/len(validset)
                valid_mse_delta_eta = valid_loss_delta_eta/len(validset)
        ...
        for _, (indices, images, _, delta_eta) in enumerate(tqdm(test_loader, ascii=True, ncols=100)):
            ...
            delta_eta_valid = torch.abs(one_hot_h_star_test - torch.softmax(outs, 1))
            test_loss_delta_eta += criterion_conf(outs, delta_eta_valid)

        valid_mse = test_loss / len(validset)
        valid_mse_delta_eta = test_loss_delta_eta / len(validset)
        cprint(f"Regression Test Loss: {valid_mse:7.3f} - Regression Test Delta Eta Mse: {valid_mse_delta_eta:7.3f}", "cyan")
