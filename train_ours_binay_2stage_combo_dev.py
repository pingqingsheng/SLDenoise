...
N_EPOCH_OUTER: int = 1
N_EPOCH_CLS:  int = 5
N_EPOCH_CALI: int = 200
CONF_RECORD_EPOCH: int = N_EPOCH_CALI - 1
LR: float = 1e-2
LR_CALI: float = 1e-4
...
def main(args):
    ...
    if args.dataset == 'mnist':
        ...
        trainset_cali = MNIST_Combo(root="./data",
                                    split="train",
    ...
    print(f"Number of cls Inner Epochs: \t {N_EPOCH_CALI}")
    ...
    model_cali = resnet18(num_classes=1, in_channels=INPUT_CHANNEL)
    gpu_id_list = [int(x) for x in args.gpus.split(",")]
    model_cali = DataParallel(model_cali, device_ids=[x-gpu_id_list[0] for x in gpu_id_list])
    model_cali = model_cali.to(device)

    optimizer_cls = torch.optim.SGD(model_cls.parameters(),
                                    lr=LR,
                                    weight_decay=WEIGHT_DECAY,
                                    momentum=0.9,
                                    nesterov=True)
    scheduler_cls = torch.optim.lr_scheduler.MultiStepLR(optimizer_cls,
                                                         gamma=0.5,
                                                         milestones=SCHEDULER_DECAY_MILESTONE)
    optimizer_cali = torch.optim.SGD(model_cali.parameters(),
                                     lr=LR_CALI,
                                     weight_decay=WEIGHT_DECAY,
                                     momentum=0.9,
                                     nesterov=True)
    scheduler_cali = torch.optim.lr_scheduler.MultiStepLR(optimizer_cali,
                                                          gamma=0.5,
                                                          milestones=SCHEDULER_DECAY_MILESTONE)

    criterion_cls = torch.nn.CrossEntropyLoss(reduction='none')
    criterion_conf = torch.nn.MSELoss()
    criterion_conf = torch.nn.HuberLoss(delta=0.3)
    ...
    for epoch in range(N_EPOCH_CALI):
        ...
        for ib, (indices, images, labels, _) in enumerate(tqdm(train_loader, ascii=True, ncols=100)):
            ...
            loss = loss_ce + 0.8*loss_cali + 0.1*loss_sm
            loss.backward()
            optimizer_cls.step()
        ...
        if not (epoch % MONITOR_WINDOW):
            ...
            print(f"Step [{epoch + 1}|{N_EPOCH_CALI}] - Train Loss:[{loss_ce.item():.3f} | {loss_cali.item():.3f}]" +
    ...
    if args.figure:
        ...
        plt.subplot(2, 2, 1)
        x_axis = np.linspace(0, N_EPOCH_CALI - N_EPOCH_CALI % MONITOR_WINDOW,
                             int((N_EPOCH_CALI - N_EPOCH_CALI % MONITOR_WINDOW) / MONITOR_WINDOW + 1))
        plt.plot(x_axis[:-1], _loss_record[:-1], linewidth=2)
        plt.title("Loss Curve")
        plt.subplot(2, 2, 2)
        plt.plot(x_axis[:-1], _valid_acc_raw_record[:-1], linewidth=2, label="Raw")
        plt.plot(x_axis[:-1], _valid_acc_cali_record[:-1], linewidth=2, label="OursDevCombo")
        plt.title('Acc Curve')
        plt.subplot(2, 2, 3)
        plt.plot(x_axis[:-1], _ece_raw_record[:-1], linewidth=2, label="Raw")
        plt.plot(x_axis[:-1], _ece_cali_record[:-1], linewidth=2, label="OursDevCombo")
        plt.legend()
        plt.title('ECE Curve')
        plt.subplot(2, 2, 4)
        plt.plot(x_axis[:-1], _l1_raw_record[:-1], linewidth=2, label="Raw")
        plt.plot(x_axis[:-1], _l1_cali_record[:-1], linewidth=2, label="OursDevCombo")
        plt.legend()
        plt.title('L1 Curve')

        time_stamp = datetime.datetime.strftime(datetime.datetime.today(), "%Y-%m-%d")
        fig.savefig(os.path.join("./figures", f"exp_log_{args.dataset}_{args.noise_type}_{args.noise_strength}_OursDevCombo_plot_{time_stamp}.png"))
...
if __name__ == "__main__":
    ...
    exp_config['n_epoch_cls'] = N_EPOCH_CALI
    exp_config['lr'] = LR
    ...
    for seed in [77, 78, 79]:
        ...
        exp_config['l1'].append(ours_l1)
        exp_config['ece'].append(ours_ece)
        exp_config['best_l1'].append(best_l1)
        exp_config['best_ece'].append(best_ece)

    dir_date = datetime.datetime.today().strftime("%Y%m%d")
    save_folder = os.path.join("./exp_logs/ourscombodev_"+dir_date)
    os.makedirs(save_folder, exist_ok=True)
    save_file_name = 'ourscombodev_' + datetime.date.today().strftime("%d_%m_%Y") + f"_{args.seed}_{args.dataset}_{args.noise_type}_{args.noise_strength}.json"
