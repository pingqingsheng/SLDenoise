...
# from network.network import resnet18, resnet34, resnet50
...
def main(args):
    ...
    for outer_epoch in range(N_EPOCH_OUTER):
        ...
        for inner_epoch in range(N_EPOCH_INNER_CLS):
            ...
            for _, (indices, images, labels, _) in enumerate(tqdm(train_loader, ascii=True, ncols=100)):
                ...
                loss_cali = criterion_conf(torch.sigmoid(T*outs[:, NUM_CLASSES:]).squeeze(), delta_prediction)
            ...
            if not (inner_epoch % MONITOR_WINDOW):
                ...
                for _, (indices, images, labels, _) in enumerate(tqdm(valid_loader, ascii=True, ncols=100)):
                    ...
                    _valid_f_cali = torch.sigmoid(T*outs[:, NUM_CLASSES:(NUM_CLASSES+1)]).detach().cpu()
            ...
            for _, (indices, images, labels, _) in enumerate(test_loader):
                ...
                _test_conf_cali = torch.sigmoid(T*outs[:, NUM_CLASSES:(NUM_CLASSES+1)]).detach().cpu()
    ...
    if args.figure:
        ...
        plt.plot(x_axis[:-1], _ece_record_ours[:-1], linewidth=2, label="Ours_weightedv1")
        plt.legend()
        plt.title('ECE Curve')
        plt.subplot(2, 2, 4)
        plt.plot(x_axis[:-1], _mse_record_conf[:-1], linewidth=2, label="Confidence v.s. Noisy")
        plt.plot(x_axis[:-1], _mse_record_ours[:-1], linewidth=2, label="Ours_weightedv1 v.s. Noisy")
        plt.plot(x_axis[:-1], _mse_record_conf_toclean[:-1], linestyle='--' , linewidth=2, label="Confidence v.s. Clean")
        plt.plot(x_axis[:-1], _mse_record_ours_toclean[:-1], linestyle='--' , linewidth=2, label="Ours_weightedv1 v.s. Clean")
        plt.legend()
        plt.title('L1 Curve')

        time_stamp = datetime.datetime.strftime(datetime.datetime.today(), "%Y-%m-%d")
        fig.savefig(os.path.join("./figures",f"exp_log_{args.dataset}_{args.noise_type}_{args.noise_strength}_binary_share_weightedv1_plot_{time_stamp}.png"))

    # return _best_naive_l1, _best_ours_l1, _best_naive_ece, _best_ours_ece
    return naive_l1.item(), ours_l1.item(), naive_ece.item(), ours_ece.item()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gpus", type=str, help="Indices of GPUs to be used", default='3')
    ...
    save_folder = os.path.join("./exp_logs/oursweightedv1_"+dir_date)
    os.makedirs(save_folder, exist_ok=True)
    save_file_name = 'oursweightedv1_' + datetime.date.today().strftime("%d_%m_%Y") + f"_{args.seed}_{args.dataset}_{args.noise_type}_{args.noise_strength}.json"
