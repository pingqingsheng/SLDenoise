...
from network.network import resnet18, resnet18gp
...
def main(args):
    ...
    model_cls = resnet18gp(in_channels=INPUT_CHANNEL, in_dim=ENCODE_DIM)
    gpu_id_list = [int(x) for x in args.gpus.split(",")]
    model_cls = DataParallel(model_cls, device_ids=[x-gpu_id_list[0] for x in gpu_id_list])
    model_cls = model_cls.to(device)
    likelihood = gpytorch.likelihoods.SoftmaxLikelihood(num_features=model_cls.module.encoder.fc.out_features,  num_classes=NUM_CLASSES).to(device)
    
    optimizer = torch.optim.SGD(
        [
            {'params':model_cls.module.encoder.parameters(), 'weight_decay':WEIGHT_DECAY}, 
            {'params':model_cls.module.gp_layer.hyperparameters(), 'lr':LR*0.01}, 
            {'params':model_cls.module.gp_layer.variational_parameters()},
            {'params':likelihood.parameters()}
        ], lr=LR, weight_decay=0, nesterov=True, momentum=0.9)
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, gamma=0.5, milestones=SCHEDULER_DECAY_MILESTONE)
    
    criterion_mll = gpytorch.mlls.VariationalELBO(likelihood, model_cls.module.gp_layer, num_data=len(trainset))
    ...
    for epoch in range(N_EPOCH_INNER_CLS):
        ...
        with gpytorch.settings.num_likelihood_samples(32) as _, gpytorch.settings.cholesky_jitter(1e-1) as _:
            for _, (indices, images, labels, _) in enumerate(tqdm(train_loader, ascii=True, ncols=100)):
                ...
                loss = -criterion_mll(outs, labels)
                loss.backward()
                optimizer.step()

                with torch.no_grad():
                    _, predict = likelihood(outs).probs.mean(0).max(1)
        ...
        if not (epoch % MONITOR_WINDOW):
            ...
            with gpytorch.settings.num_likelihood_samples(32) as _ , gpytorch.settings.cholesky_jitter(1e-1) as _:
                for _, (indices, images, labels, _) in enumerate(tqdm(valid_loader, ascii=True, ncols=100)):
                    ...
                    with torch.no_grad():
                        prob_outs = likelihood(outs_raw).probs.mean(0)
            ...
            model_cls.eval()
            likelihood.eval()
            with gpytorch.settings.num_likelihood_samples(32) as _ , gpytorch.settings.cholesky_jitter(1e-2) as _:
            ...
            print(">>> Debugg")
            print(np.round(test_f_cali[:5].numpy(), 3), '\n')
            ...
            print(f"ECE Cali: {ece_loss_cali.item():.3f}/{_best_cali_ece.item():.3f} - Best ECE Cali Epoch: {_best_cali_ece_epoch}")
            print(f"L1 Cali: {l1_loss_cali.item():.3f}/{_best_cali_l1.item():.3f} - Best L1 Cali: {_best_cali_l1_epoch}")
    ...
    if args.figure:
        ...
        plt.subplot(2, 2, 2)
        plt.plot(x_axis[:-1], _valid_acc_cali_record[:-1], linewidth=2, label="GP")
        plt.title('Acc Curve')
        plt.subplot(2, 2, 3)
        plt.plot(x_axis[:-1], _ece_cali_record[:-1], linewidth=2, label="GP")
        plt.legend()
        plt.title('ECE Curve')
        plt.subplot(2, 2, 4)
        plt.plot(x_axis[:-1], _l1_cali_record[:-1], linewidth=2, label="GP")
        plt.legend()
        plt.title('L1 Curve')

        time_stamp = datetime.datetime.strftime(datetime.datetime.today(), "%Y-%m-%d")
        fig.savefig(os.path.join("./figures", f"exp_log_{args.dataset}_{args.noise_type}_{args.noise_strength}_GP_plot_{time_stamp}.png"))

    # >>> Debugging
    import pickle as pkl
    
    debug_folder = "../debug/gpc"
    os.makedirs(debug_folder, exist_ok=True)
    time_stamp = datetime.datetime.strftime(datetime.datetime.today(), "%Y%m%d%H%M%S")
    debuglogger = os.path.join(debug_folder, f"gpc_{args.dataset}_{args.noise_type}_{args.noise_strength}_{time_stamp}.pkl")
    print(debuglogger)
    ...
    return l1_loss_cali.item(), _best_cali_l1.item(), ece_loss_cali.item(), _best_cali_ece.item(), test_acc_cali, _best_cali_acc
...
if __name__ == "__main__":
    ...
    save_folder = os.path.join("../exp_logs/gpc_" + dir_date)
    os.makedirs(save_folder, exist_ok=True)
    save_file_name = 'gpc_' + datetime.date.today().strftime("%d%m%Y%H%M%S") + f"{args.dataset}_{args.noise_type}_{args.noise_strength}.json"
